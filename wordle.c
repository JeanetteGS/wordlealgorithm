...
// Checks where guess and word have the same letter and stores them
void mystrpos(const char *g, const char *word, char *rl);
...
void myGameLoop(void)
{
	...
	char guessSix[WORD_LENGTH + 1] = {0};
	char rightLetters[WORD_LENGTH + 1] = {0};
	...
	mystrpos(guess, word, rightLetters); 
	mystrpos(guessTwo, word, rightLetters);
	mystrpos(guessThree, word, rightLetters);
	if(guessFour != NULL){
		mystrpos(guessFour, word, rightLetters);
	}

	//To check what is in letter array
	printf("Letter Array= %s\n", rightLetters);

	// the next guess needs to include the letters that the first guesses got correct
	ndx = refinedSearch(rightLetters);
	...
	//Guess 6
	ndx = refinedSearch(rightLetters);
	strcpy(guessSix, candidates[ndx]);
	printf("Guess 6: %s" EOL, guessSix);
	if (strcmp(guessSix, word)){
		if (checkWord(guessSix)){
			removeAlpha(guessSix);
			printf("\t%s\n", alpha);
		}
	}else{
		puts("Computer Wins");
...
int refinedSearch(char *rl){
	...
	while (i < ALL_WORDS && fgets(candidates[i], WORD_LENGTH + 1, fpAll) != NULL){
		...
		for(int a = 0; a < WORD_LENGTH; a++){
			for(int b = 0; b < rl_len; b++){
				if(candidates[i][a] == rl[b]){
					isCandidate++;
				}
			}
		}
		if(isCandidate == rl_len){
		...
		if(isCandidate == WORD_LENGTH){
			printf("isCandidate= %d", isCandidate);
			return i;
		}
	}
	return i;
}
void mystrpos(const char *g, const char *word, char *rl){ //pass in rightletters array to store the letters in guess[pos[i]]
	int k = 0, dup_count = 0, dup_present = 0;
	while (rl[k])
		k++;
	//bool duplicateLetters;
...
bool checkWord(const char *guess)
{
	if (strlen(guess) == strlen(word))
	{
		...
		while (copy[i])
		{
			...
			if (pos >= 0 && result[i] != '#')
			{ // pos is wherever word and guess have the same letter, puts an 'o' in that position and blanks it out on word
...
int pickWord(char *word, int index)
{
	...
	{ // fgets stores the solution into word
		if (strlen(word) == WORD_LENGTH)
		{
			++i;
		}
